    ...
    public static class Program {
        ...
        private static void FinishDraft(string[] draft, IDictionary<int, string> labeledLines, IDictionary<int, string> pointersLines) {
            foreach (var pointerLine in pointersLines) {
                ...
                if (instructionParts[0] == "op" && instructionParts.Length >= 5 && instructionParts[1] == ParameterToSubstitute.ToString() && instructionParts[4] == ParameterToSubstitute.ToString() && instructionParts[3] == "@counter") {
                    int index;
                    string operation;
